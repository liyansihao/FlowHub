...
try {
 ...
 const allowed={'/api.product.favorite/lists':'GET','/api.product.favorite/toggle':'POST','/api.product.favorite/edit_import':'POST','/api.product.collect/detail':'GET','/api.product.collect/lists':'GET'};
 ...
 if(r.json?.code!==1)throw Object.assign(Error('ERP source acquisition rejected'), {code:'ERP_SOURCE_REJECTED',status:r.status,apiCode:r.json?.code,apiMessage:String(r.json?.msg||r.json?.message||'').replaceAll(process.env.MAOZI_ACCESS_TOKEN||'__no_token__','[redacted]').slice(0,200),operation:c.path});
 ...
 process.stdout.write(JSON.stringify({ok:false,error:code,diagnostic:{
  ...
  api_code:error.apiCode??null,api_message:error.apiMessage||null,operation:error.operation||null,

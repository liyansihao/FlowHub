...
async function api(mode='view',body){const r=await fetch('/api/operations?mode='+mode,{method:body?'POST':'GET',headers:{'Content-Type':'application/json','X-Ops-Request':'1'},signal:AbortSignal.timeout(30000),...(body?{body:JSON.stringify(body)}:{})});const d=await r.json();if(!r.ok){throw Error(d.error||'请求失败');}return d;}
function error(e){$('#error').textContent=e.message;$('#error').hidden=false;}
function render(){
 const store=$('#store').value,handled=$('#handled').value,q=$('#search').value.trim().toLowerCase(),stage=$('#stage').value;
 ...
 const rows=[...data.items].sort((a,b)=>String(b.created_at||'').localeCompare(String(a.created_at||''))).filter(r=>r.kind===kind&&(!stage||kind!=='returns'||(r.return_stage||'unknown')===stage)&&(!store||r.store_id===store)&&(handled==='all'||r.handling.handled===(handled==='done'))&&(!q||[r.name,r.offer_id,r.order,r.sku,r.store_name,r.reason,r.reason_cn].join(' ').toLowerCase().includes(q)));
 ...
 return `<tr><td>${sku?`<a href="https://www.ozon.ru/product/${sku}/" target="_blank" rel="noreferrer">${esc(r.name||r.offer_id)} ↗</a>`:esc(r.name||r.offer_id)}<div class="sub">${esc(r.order||r.offer_id||r.id)}</div>${kind==='returns'?`<div class="reason">${esc(r.reason_cn||'原因待核实')}</div><div class="sub">${esc(r.return_stage_label||'订单阶段待核实')} · 来源：${esc(r.reason_source||'待读取')}</div><details class="sub"><summary>原因原文与核验依据</summary><p>${esc(r.reason||'平台未提供')}</p>${r.buyer_comment?`<p>买家补充：${esc(r.buyer_comment)}</p>`:''}${r.order_cancel_reason&&r.order_cancel_reason!==r.reason?`<p>订单取消原因：${esc(r.order_cancel_reason)}</p>`:''}<p>${esc(r.stage_evidence||'未取得订单核验结果')}</p><p>核验时间：${time(r.detail_checked_at)}</p>${r.detail_error?`<p class="warning">${esc(r.detail_error)}</p>`:''}</details>`:''}</td><td>${esc(r.store_name)}</td><td>${kind==='inventory'?'<span class="warning">库存 0</span>':esc(r.state_label||r.state)}${kind==='returns'?`<div class="sub">申请 ${time(r.created_at)}</div>`:''}</td><td>${time(r.synced)}${stale?'<div class="sub warning">数据待更新</div>':''}</td><td>${r.handling.handled?`<div class="done">✓ 已经处理</div><div class="sub">${time(r.handling.at)}</div>`:''}<button class="${r.handling.handled?'undo':'primary'}" data-key="${esc(r.key)}" ${busy?'disabled':''}>${r.handling.handled?'撤销处理':'已经处理'}</button></td></tr>`;
 }).join('')||'<tr><td colspan="5" class="empty">当前筛选下没有记录</td></tr>';
}
async function load(){if(busy)return;data=await api();$('#workspace').hidden=false;$('#error').hidden=true;const selected=$('#store').value;$('#store').innerHTML='<option value="">全部店铺</option>'+data.stores.map(s=>`<option value="${esc(s.id)}">${esc(s.name)}</option>`).join('');$('#store').value=selected;
...
document.querySelectorAll('[data-kind]').forEach(b=>b.onclick=()=>{kind=b.dataset.kind;$('#stage').hidden=kind!=='returns';page=0;document.querySelectorAll('[data-kind]').forEach(x=>x.setAttribute('aria-pressed',String(x===b)));render();});
for(const id of ['store','handled','stage','search'])$('#'+id).addEventListener(id==='search'?'input':'change',()=>{page=0;render();});
